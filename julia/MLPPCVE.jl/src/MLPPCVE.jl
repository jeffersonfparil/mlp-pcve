...
function args_parser()
    ...
    args_output["expected-labels-A"] =
        String.(split(strip(args["expected-labels"], strip_these_chars), ","))
    args_output["requested-contextualised-effects"] =
        String.(split(strip(args["requested-interaction-effects"], strip_these_chars), ","))
    ...
    # Vectors of Float64
    for k in ["opt-dropout-per-hidden-layer"]
        args_output[k] = begin
            a = Float64[]
            for b in split(strip(args[k], strip_these_chars), ",")
                try
    ...
    for k in ["opt-activation-functions", "opt-cost-functions"]
        args_output[k] = begin
    ...
    args_output["opt-optimisers"] = begin
        ...
        for optimiser in optimisers
            if optimiser ∉ ["GD", "Adam", "AdamMax"]
                error(
                    "Unrecognised optimiser: $optimiser. Please choose from \"GD\", \"Adam\", and \"AdamMax\".",
                )
            end
        end
        optimisers
