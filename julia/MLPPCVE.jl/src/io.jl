...
function simulatevars(;
    ...
    n_blocks = squarest(n_plots)[1],
    n_plots_per_block = squarest(n_plots)[2],
    n_block_rows = squarest(n_blocks)[1],
    n_block_cols = squarest(n_blocks)[2],
    n_rows_per_block = squarest(n_plots_per_block)[1],
    n_cols_per_block = squarest(n_plots_per_block)[2],
    ...
    explanatory_variables = Dict(
        ...
        :populations_cor_entries => [["population_$(string(i, pad=length(string(n_populations))))" for i in 1:n_populations][Int(ceil(rand()*n_populations))] for _ in 1:n_entries],
        :populations => ["population_$(string(i, pad=length(string(n_populations))))" for i in 1:n_populations],
    ...
    for var in [:years, :seasons, :harvests, :sites, :replications, :entries, :populations, :blocks, :rows, :cols]
        ...
        for idx_seasons in findall(.!isnothing.(match.(Regex("^seasons"), labels_X)))
            for idx_harvests in findall(.!isnothing.(match.(Regex("^harvests"), labels_X)))
                for idx_sites in findall(.!isnothing.(match.(Regex("^sites"), labels_X)))
                    ...
                    idx_row_sub = idx_row
                    for idx_replications in findall(.!isnothing.(match.(Regex("^replications"), labels_X)))
                        for idx_entries in findall(.!isnothing.(match.(Regex("^entries"), labels_X)))
                            idx_replications_last = maximum(findall(.!isnothing.(match.(Regex("^replications"), labels_X))))
                            idx_entries_last = maximum(findall(.!isnothing.(match.(Regex("^entries"), labels_X))))
                            idx_populations = idx_entries_last + findall(
                                explanatory_variables[:populations] .== explanatory_variables[:populations_cor_entries][idx_entries - idx_replications_last]
                            )[1]
    ...
    labels_A = ["years", "seasons", "harvests", "sites", "replications", "entries", "populations", "blocks", "rows", "cols"]
    k = length(labels_A)
    ...
    for k in Symbol.(labels_A)
        ...
        A[:, idx_A] = [v[findfirst(X[i, idx_X] .== 1.0)] for i in 1:n]
    end
    # Output
    Dict(
        "X"=>X, 
...
function simulatelineartrait(
    ...
    for (k, v) in linear_effects
        ...
        β[idx] = sampleNormal(length(idx), μ=linear_effects[k].μ, σ=linear_effects[k].σ, seed=seed)
    end
    y = X * β
...
function simulatenonlineartrait(
    ...
    Ω = init(CuArray(Matrix(X')), n_hidden_layers=n_hidden_layers, F=relu, dropout_rates=dropout_rates, seed=seed)
    rng = CUDA.seed!(seed)
...
function simulatedata(;linear::Bool=false)::Dict{String}
    D = simulatevars()
    y = if linear
        simulatelineartrait(D["X"], labels_X=D["labels_X"])
    else
        simulatenonlineartrait(D["X"])
    end
    Dict(
        "A" => D["A"],
        "y" => y,
