...
function readtrial(;
    ...
    if length(header) < 2
        error(
            "File \"$fname\" has less than 2 columns. Delimiter: \"$delimiter\" may not be correct.",
        )
    end
    close(io)
    labels_A = String[]
    ...
    open(fname, "r") do io
        ...
        @inbounds for i = 1:n
            ...
            if length(ay) != length(header)
                error(
                    "Row $i in file \"$fname\" does not have the same number of columns as the header. Delimiter: \"$delimiter\" may not be correct.",
                )
            end
            y = try
                ...
                error(
                    "Column/s $(join(labels_Y, ", ")) in file \"$fname\" cannot be parsed as Float64.",
                )
            end
            A[i, :] = String.(ay[idx_A])
            Y[i, :] = y
    ...
    Trial(X, Y, A, labels_X, labels_Y, labels_A)
end
