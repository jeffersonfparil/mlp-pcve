...
function main(args::Dict)::Vector{String}
    ...
    idx = findall(
        .!isnothing.(match.(Regex("^$(output_prefix)-"), fnames_output)) .&&
        .!isnothing.(match.(Regex(".jld2\$"), fnames_output)),
    )
    if length(idx) > 0
        error(
            "Please re/move the existing output file/s with the prefix: \"$(output_prefix)\".",
        )
    end
    # Read the trial data
    trial = readtrial(
        fname = fname,
        delimiter = delimiter,
        expected_labels_A = expected_labels_A,
    )
    ...
    traits = if (length(traits) == 1) && (traits[1] == "")
        ...
        for trait in traits
            if trait ∉ trial.labels_Y
                error(
                    "The supplied trait: \"$trait\" does not exist in the input file: $fname. Valid traits are: \n\t‣ $(join(trial.labels_Y, "\nt\t‣ "))",
                )
            end
        end
        traits
    ...
    idx_vars = sort(
        vcat(
            [
                findall(.!isnothing.(match.(Regex(x), trial.labels_X))) for
                x in nonfixed_explanatory_variables
            ]...,
        ),
    ) # remove fixed variables
    if length(idx_vars) < 2
        error(
            "There is less than 2 non-fixed explanatory variables:\n\t‣ $(join(trial.labels_X[idx_vars], "\n\t‣ "))",
        )
    end
    if length(idx_vars) < length(trial.labels_X)
        idx_tmp = findall([!(x ∈ idx_vars) for x in collect(1:length(trial.labels_X))])
        @warn(
            "Dropped the following explanatory variable level/s:\n\t‣ $(join(trial.labels_X[idx_tmp], "\n\t‣ "))"
        )
    end
    ...
    for (j, trait) in enumerate(traits)
        ...
        if length(y) < 10
            @warn(
                "Skipped trait \"$trait\" because it does not have enough non-missing observations."
            )
            continue
        end
        if σ_y < 1e-12
        ...
        Xy::Dict{String,CuArray{T,2}} =
            Dict("X" => CuArray(T.(trial.X[idx_obs, idx_vars]')), "y" => CuArray(T.(y')))
        dl_optim = optim(
            Xy,
            n_batches = n_batches,
        ...
        for contextualised_effect in requested_contextualised_effects
            ...
            idx_factors = [
                findall(.!isnothing.(match.(Regex("^$factor"), labels_X))) for
                factor in factors
            ]
            ...
            Φ_context[contextualised_effect] =
                Dict("ϕ_context" => ϕ_tmp[:, 1], "ϕ_context_labels" => ϕ_tmp_labels)
        end
        # Outputs
        output = Dict(
            "trait" => trait,
        ...
        fnames_outputs = vcat(
            ...
            [
                "$(output_prefix)-$(trait)-$(replace(k, "-" => "_"))_standardised_effects.tsv"
                for k in keys(Φ_context)
            ],
        )
        ...
        open(fnames_outputs[2], "w") do file
            ...
            write(
                file,
                join(
                    string.(replace.(ϕ_nocontext_labels, "|" => "\t"), "\t", ϕ_nocontext),
                    "\n",
                ),
            )
        end
        ...
        for (i, (k, v)) in enumerate(Φ_context)
            ...
            open(fnames_outputs[i+2], "w") do file
                ...
                write(
                    file,
                    join(string.(v["ϕ_context_labels"], "\t", v["ϕ_context"]), "\n"),
                )
            end
        end
        println("##################################################")
