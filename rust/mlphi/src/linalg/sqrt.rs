...
const ELEMENTWISESQRT: &str = "
    extern \"C\" __global__ void cuElementwiseSqrt(float* A, float* B, int n_rows, int n_cols) {
        // Element-wise square-root kernel implementation
        // Arguments:
...
impl Matrix {
    pub fn elementwisematsqrt(self: &Self) -> Result<Self, Box<dyn Error>> {
        let ptx: Ptx = compile_ptx(ELEMENTWISESQRT)?;
        let ctx: Arc<CudaContext> = CudaContext::new(0)?;
        let stream: Arc<CudaStream> = ctx.default_stream();
        let module: Arc<CudaModule> = ctx.load_module(ptx)?;
        let f: CudaFunction = module.load_function("cuElementwiseSqrt")?;
...
mod tests {
    ...
    fn test_sqrt() -> Result<(), Box<dyn Error>> {
        ...
        let matrix_1 = a_matrix.elementwisematsqrt()?;
        ...
        println!("After `elementwisematsqrt`: a_host {:?}", a_host);
        assert_eq!(
            a_host,
            vec![
                0.0_f32.sqrt(),
