...
const ELEMENTWISEABS: &str = "
    extern \"C\" __global__ void cuElementwiseAbs(float* A, float* B, int n_rows, int n_cols) {
        // Element-wise absolute value kernel implementation
        // Arguments:
...
impl Matrix {
    pub fn elementwisematabs(self: &Self) -> Result<Self, Box<dyn Error>> {
        ...
        let f: CudaFunction = module.load_function("cuElementwiseAbs")?;
...
mod tests {
    use super::*;
    #[test]
    fn test_abs() -> Result<(), Box<dyn Error>> {
        ...
        let matrix_1 = a_matrix.elementwisematabs()?;
        ...
        println!("After `elementwisematabs`: a_host {:?}", a_host);
        assert_eq!(
            a_host,
            vec![
                0.0_f32.abs(),
